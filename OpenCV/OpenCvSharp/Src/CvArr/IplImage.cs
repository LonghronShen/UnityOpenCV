...
namespace OpenCvSharp
{
    ...
    { 
        ...
        public IplImage(string filename, LoadMode flags)
        {
            ...
            this._ptr = CvInvoke.cvLoadImage(filename, flags);
        ...
        {
            this._ptr = CvInvoke.cvCreateImage(size, depth, channels);
            if (this._ptr == IntPtr.Zero)
            {
                throw new OpenCvSharpException("Failed to create IplImage");
            }
...
//        }
        ...
        /// <returns></returns>
#endif
        public static IplImage FromStream(Stream stream)
        {
            if (stream == null)
            {
...
#endif
        public void SnakeImage(CvPoint[] points, float alpha, float beta, float gamma, CvSize win, CvTermCriteria criteria, bool calc_gradient)
        {
            Cv.SnakeImage(this, points, alpha, beta, gamma, win, criteria, calc_gradient);
        }
#if LANG_JP
        /// <summary>
        /// 内部エネルギーと外部エネルギーの総和が最小になるように snake を更新する． 
...
//        }
        ...
        {
            BitmapConverter.DrawToHdc(this, hdc, x, y, w, h, from_x, from_y);
        }
...
//        {
//            BitmapConverter.DrawToGraphics(this, g, x, y, w, h, from_x, from_y);
//        }
        #endregion        
        #region GetSubImage
#if LANG_JP
        ...
        public void GetObjectData(SerializationInfo info, StreamingContext context)
        {
            ...
            const int Length = 1024 * 512;
            byte[] buffer = new byte[Length];
            using (MemoryStream ms = new MemoryStream())
            using (DeflateStream gzs = new DeflateStream(ms, CompressionMode.Compress))
            {                
                for (int offset = 0; offset < ImageSize; offset += Length)
                {
                    int length = (offset + Length < ImageSize) ? Length : ImageSize - offset;
        ...
        protected IplImage(SerializationInfo info, StreamingContext context)
        {
            ...
            // 画像領域を準備
            this._ptr = CvInvoke.cvCreateImage(size, depth, nChannels);
            if (this._ptr == IntPtr.Zero)
            {
                throw new OpenCvSharpException("Failed to create IplImage");
            }
