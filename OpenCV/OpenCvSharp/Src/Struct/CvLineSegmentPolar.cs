...
namespace OpenCvSharp
{
    ...
    public struct CvLineSegmentPolar : IEquatable<CvLineSegmentPolar>
    {
        ...
        /// <returns></returns>
#endif
        public static CvPoint? LineIntersection(CvLineSegmentPolar line1, CvLineSegmentPolar line2)
        {
        ...
        /// <returns></returns>
#endif
        public CvPoint? LineIntersection(CvLineSegmentPolar line)
        {
        ...
        public CvLineSegmentPoint ToSegmentPoint(double scale)
        {
            ...
            double x0 = cos * Rho;
            double y0 = sin * Rho;
            CvPoint p1 = new CvPoint { X = (int)Math.Round(x0 + scale * -sin), Y = (int)Math.Round(y0 + scale * cos) };
            CvPoint p2 = new CvPoint { X = (int)Math.Round(x0 - scale * -sin), Y = (int)Math.Round(y0 - scale * cos) };
        ...
        {
            CvLineSegmentPolar axis = new CvLineSegmentPolar(y, (float)(Cv.PI / 2));     // 垂線90度 = x軸に平行       
            CvPoint? node = LineIntersection(axis);
            if (node.HasValue)
                return node.Value.X;
            else
                return null;
        ...
        {
            CvLineSegmentPolar axis = new CvLineSegmentPolar(x, (float)0);     // 垂線0度 = y軸に平行       
            CvPoint? node = LineIntersection(axis);
            if (node.HasValue)
                return node.Value.Y;
            else
