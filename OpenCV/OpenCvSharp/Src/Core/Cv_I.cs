...
namespace OpenCvSharp
{
    public static partial class Cv
    {
...
#endif
        public static void InitFont(out CvFont font, FontFace font_face, double hscale, double vscale)
        {
            font = new CvFont(font_face, hscale, vscale);
        }
#if LANG_JP
        /// <summary>
        /// 文字描画関数に渡されるフォント構造体を初期化する
...
#endif
        public static void InitFont(out CvFont font, FontFace font_face, double hscale, double vscale, double shear)
        {
            font = new CvFont(font_face, hscale, vscale, shear);
        }
#if LANG_JP
        /// <summary>
        /// 文字描画関数に渡されるフォント構造体を初期化する
...
#endif
        public static void InitFont(out CvFont font, FontFace font_face, double hscale, double vscale, double shear, int thickness)
        {
            font = new CvFont(font_face, hscale, vscale, shear, thickness);
        }
#if LANG_JP
        /// <summary>
        /// 文字描画関数に渡されるフォント構造体を初期化する
...
#endif
        public static void InitFont(out CvFont font, FontFace font_face, double hscale, double vscale, double shear, int thickness, LineType line_type)
        {
            font = new CvFont(font_face, hscale, vscale, shear, thickness, line_type);
        }
        #endregion
        ...
        {
            Cv.InitIntrinsicParams2D(objectPoints, imagePoints, npoints, imageSize, cameraMatrix, 1.0);
        }
#if LANG_JP
        /// <summary>
        /// 3次元-2次元の対応点から，カメラの内部パラメータ行列を求めます
        ...
        /// <param name="line_iterator">ラインイテレータ状態構造体へのポインタ</param>
        /// <returns></returns> 
#else
        /// <summary>
...
#endif
        public static int InitLineIterator(CvArr image, CvPoint pt1, CvPoint pt2, out CvLineIterator line_iterator)
        {
            return InitLineIterator(image, pt1, pt2, out line_iterator, PixelConnectivity.Connectivity_8);
        }
#if LANG_JP
        /// <summary>
        /// ラインイテレータを初期化する
        ...
        /// <returns>The function cvInitLineIterator initializes the line iterator and returns the number of pixels between two end points. Both points must be inside the image. After the iterator has been initialized, all the points on the raster line that connects the two ending points may be retrieved by successive calls of NextLinePoint point. The points on the line are calculated one by one using 4-connected or 8-connected Bresenham algorithm.</returns> 
#endif
        public static int InitLineIterator(CvArr image, CvPoint pt1, CvPoint pt2, out CvLineIterator line_iterator, PixelConnectivity connectivity)
        {
            return InitLineIterator(image, pt1, pt2, out line_iterator, connectivity, false);
        }
#if LANG_JP
        /// <summary>
        /// ラインイテレータを初期化する
        ...
        /// <returns>The function cvInitLineIterator initializes the line iterator and returns the number of pixels between two end points. Both points must be inside the image. After the iterator has been initialized, all the points on the raster line that connects the two ending points may be retrieved by successive calls of NextLinePoint point. The points on the line are calculated one by one using 4-connected or 8-connected Bresenham algorithm.</returns> 
#endif
        public static int InitLineIterator(CvArr image, CvPoint pt1, CvPoint pt2, out CvLineIterator line_iterator, PixelConnectivity connectivity, bool left_to_right)
        {
            if (image == null)
                throw new ArgumentNullException("image");
            line_iterator = new CvLineIterator(image, pt1, pt2, connectivity, left_to_right);
            return line_iterator.Count;
        }
        #endregion
        ...
        /// <param name="mat_iterator">初期化されるイテレータ</param>
...
#endif
        public static CvSparseNode InitSparseMatIterator(CvSparseMat mat, out CvSparseMatIterator mat_iterator)
        {
            if (mat == null)
            {
                throw new ArgumentNullException("mat");
            }
            mat_iterator = new CvSparseMatIterator();
            return mat_iterator.Init(mat);
        }
        #endregion
        ...
        /// <param name="max_level">ツリー走査範囲の最大レベル（first ノードが第1レベルであると仮定する）．
...
#endif
        public static void InitTreeNodeIterator<T>(out CvTreeNodeIterator tree_iterator, CvTreeNode<T> first, int max_level)
            where T : CvTreeNode<T>
        {
            ...
            tree_iterator = new CvTreeNodeIterator();
            CvInvoke.cvInitTreeNodeIterator(tree_iterator, first.CvPtr, max_level);
        }
        #endregion
...
#endif
        public static void InitUndistortMap(CvMat intrinsic_matrix, CvMat distortion_coeffs, CvArr mapx, CvArr mapy)
        {
            ...
            CvInvoke.cvInitUndistortMap(intrinsic_matrix.CvPtr, distortion_coeffs.CvPtr, mapx.CvPtr, mapy.CvPtr);
        }
        #endregion
        ...
        /// <param name="new_camera_matrix">新しいカメラ行列 A'=[fx' 0 cx'; 0 fy' cy'; 0 0 1]． </param>
        ...
        /// <param name="mapy">The output array of y-coordinates of the map. </param>
#endif
        public static void InitUndistortRectifyMap(CvMat camera_matrix, CvMat dist_coeffs, CvMat R, CvMat new_camera_matrix, CvArr mapx, CvArr mapy)
        {
            if (camera_matrix == null)
                throw new ArgumentNullException("camera_matrix");
            if (dist_coeffs == null)
                throw new ArgumentNullException("dist_coeffs");
            if (new_camera_matrix == null)
                throw new ArgumentNullException("new_camera_matrix");
            if (mapx == null)
                throw new ArgumentNullException("mapx");
            if (mapy == null)
                throw new ArgumentNullException("mapy");
            IntPtr R_ptr = (R == null) ? IntPtr.Zero : R.CvPtr;
            CvInvoke.cvInitUndistortRectifyMap(camera_matrix.CvPtr, dist_coeffs.CvPtr, R_ptr, new_camera_matrix.CvPtr, mapx.CvPtr, mapy.CvPtr);
        }
        #endregion
        #region Inpaint
#if LANG_JP
...
#endif
        public static void Integral(CvArr image, CvArr sum, CvArr sqsum, CvArr tilted_sum)
        {
            if (image == null)
                throw new ArgumentNullException("image");
            if (sum == null)
            ...
            CvInvoke.cvIntegral(image.CvPtr, sum.CvPtr, sqsum_ptr, tilted_sum_ptr);
        }
        #endregion
        #region Invert
#if LANG_JP
        ...
        {
            int depth = Cv.MAT_DEPTH(type);
            return (BitDepth)(Cv.ELEM_SIZE1(depth) * 8 | (int)(depth == CvConst.CV_8S || depth == CvConst.CV_16S || depth == CvConst.CV_32S ? CvConst.IPL_DEPTH_SIGN : 0));
        }
        #endregion
        #region IsInf
#if LANG_JP
