...
namespace OpenCvSharp
{
    public static partial class Cv
    {
        ...
        /// <returns></returns>
#endif
        public static void NextLinePoint(CvLineIterator line_iterator)
        {
            line_iterator.NextLinePoint();
        }
        #endregion
        #region NextTreeNode
#if LANG_JP
        ...
        /// <returns></returns>
#endif
        public static IntPtr NextTreeNode(CvTreeNodeIterator tree_iterator)
        {
            if (tree_iterator == null)
            {
                throw new ArgumentNullException("tree_iterator");
            }
            return CvInvoke.cvNextTreeNode(tree_iterator);
        }
#if LANG_JP
        /// <summary>
        /// 現在のノードを返し，イテレータを次のノードに移動させる．
        ...
        /// <returns></returns>
#endif
        public static T NextTreeNode<T>(CvTreeNodeIterator tree_iterator) where T : CvArr
        {
            if (tree_iterator == null)
            {
                throw new ArgumentNullException("tree_iterator");
            }
            IntPtr result = CvInvoke.cvNextTreeNode(tree_iterator);
        ...
        /// <returns></returns>
#endif
        public static T NextTreeNode<T>(CvTreeNodeIterator<T> tree_iterator) where T : CvArr
        {
            return NextTreeNode<T>((CvTreeNodeIterator)tree_iterator);
        }
        #endregion
        #region Norm
#if LANG_JP
        ...
        /// <returns></returns>
#endif
        public static double Norm(CvArr arr1, CvArr arr2, NormType norm_type)
        {
            return Norm(arr1, arr2, norm_type, null);
        }
#if LANG_JP
        /// <summary>
        /// 配列の絶対値ノルム（absolute array norm），絶対値差分ノルム（absolute difference norm），相対値差分ノルム（relative difference norm）を計算する
        ...
        /// <returns></returns>
#endif
        public static double Norm(CvArr arr1, CvArr arr2, NormType norm_type, CvArr mask)
        {
            if (arr1 == null)
            {
            ...
            return CvInvoke.cvNorm(arr1.CvPtr, arr2Ptr, norm_type, maskPtr);
        }
        #endregion
        #region Normalize
#if LANG_JP
...
#endif
        public static void Normalize(CvArr src, CvArr dst, double a, double b, NormType norm_type)
        {
            Normalize(src, dst, a, b, norm_type, null);
        }
#if LANG_JP
        /// <summary>
        /// 指定のノルムになるように，あるいは値が指定の範囲になるように，配列を正規化する
...
#endif
        public static void Normalize(CvArr src, CvArr dst, double a, double b, NormType norm_type, CvArr mask)
        {
            if (src == null)
            ...
            IntPtr maskPtr = (mask == null) ? IntPtr.Zero : mask.CvPtr;
            CvInvoke.cvNormalize(src.CvPtr, dst.CvPtr, a, b, norm_type, maskPtr);
        }
        #endregion
