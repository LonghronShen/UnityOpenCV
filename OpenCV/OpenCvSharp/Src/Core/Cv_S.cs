...
namespace OpenCvSharp
{
    public static partial class Cv
    {
...
#endif
        public static void Save(string filename, IntPtr struct_ptr)
        {
            Save(filename, struct_ptr, null, null, new CvAttrList());
        }
#if LANG_JP
        /// <summary>
        /// オブジェクトをファイルに保存する
...
#endif
        public static void Save(string filename, IntPtr struct_ptr, string name)
        {
            Save(filename, struct_ptr, name, null, new CvAttrList());
        }
#if LANG_JP
        /// <summary>
        /// オブジェクトをファイルに保存する
...
#endif
        public static void Save(string filename, IntPtr struct_ptr, string name, string comment)
        {
            Save(filename, struct_ptr, name, comment, new CvAttrList());
        }
#if LANG_JP
        /// <summary>
        /// オブジェクトをファイルに保存する
...
#endif
        public static void Save(string filename, IntPtr struct_ptr, string name, string comment, CvAttrList attributes)
        {
            if (string.IsNullOrEmpty(filename))
                throw new ArgumentNullException("filename");
            if (struct_ptr == IntPtr.Zero)
                throw new ArgumentNullException("struct_ptr");
            CvInvoke.cvSave(filename, struct_ptr, name, comment, attributes);
        }
        #endregion
        #region ICvPtrHolder
#if LANG_JP
        ...
        /// <param name="filename">File name. </param>
#endif
        public static void Save(ICvPtrHolder struct_ptr, string filename)
        {
            Save(struct_ptr, filename, null, null, new CvAttrList());
        }
#if LANG_JP
        /// <summary>
        /// オブジェクトをファイルに保存する
        ...
        /// <param name="name">Optional object name. If it is null, the name will be formed from filename. </param>
#endif
        public static void Save(ICvPtrHolder struct_ptr, string filename, string name)
        {
            Save(struct_ptr, filename, name, null, new CvAttrList());
        }
#if LANG_JP
        /// <summary>
        /// オブジェクトをファイルに保存する
        ...
        /// <param name="comment">Optional comment to put in the beginning of the file. </param>
#endif
        public static void Save(ICvPtrHolder struct_ptr, string filename, string name, string comment)
        {
            Save(struct_ptr, filename, name, comment, new CvAttrList());
        }
#if LANG_JP
        /// <summary>
        /// オブジェクトをファイルに保存する
        ...
        /// <param name="attributes">Optional attributes passed to cvWrite. </param>
#endif
        public static void Save(ICvPtrHolder struct_ptr, string filename, string name, string comment, CvAttrList attributes)
        {
            if (struct_ptr == null)
            {
                throw new ArgumentNullException("struct_ptr");
            }
            Save(filename, struct_ptr.CvPtr, name, comment, attributes);
        }
        #endregion
        #endregion
        #region SaveImage
        ...
        public static int SaveImage(string filename, CvArr image, int[] prms)
        {
            ...
            if (image == null)
                throw new ArgumentNullException("image");
            return CvInvoke.cvSaveImage(filename, image.CvPtr, prms);
        }
#if LANG_JP
        /// <summary>
        /// 画像を指定したファイルに保存する．画像フォーマットは，filename の拡張子により決定される．
        ...
        {
            ScaleAdd(A, CvScalar.RealScalar(real_scalar), B, C);
        }
        #endregion
...
#endif
        public static CvSeq SegmentMotion(CvArr mhi, CvArr seg_mask, CvMemStorage storage, double timestamp, double seg_thresh)
        {
            if (mhi == null)
                throw new ArgumentNullException("mhi");
            if (storage == null)
                throw new ArgumentNullException("storage");
            return new CvSeq(CvInvoke.cvSegmentMotion(mhi.CvPtr, seg_mask.CvPtr, storage.CvPtr, timestamp, seg_thresh));
        }
        #endregion
        #region SeqElemIdx
#if LANG_JP
...
#endif
        public static T SeqInsert<T>(CvSeq seq, int before_index, T element) where T : struct
        {
            if (seq == null)
            {
                throw new ArgumentNullException("seq");
            ...
            //if (typeof(T).IsValueType)
            {
                using (StructurePointer<T> elementPtr = new StructurePointer<T>(element))
                {
                    IntPtr ptr = CvInvoke.cvSeqInsert(seq.CvPtr, before_index, elementPtr);
...
#endif
        public static void SeqInsertSlice(CvSeq seq, int before_index, CvArr from_arr)
        {
            if (seq == null)
                throw new ArgumentNullException("seq");
            if (from_arr == null)
                throw new ArgumentNullException("from_arr");
            CvInvoke.cvSeqInsertSlice(seq.CvPtr, before_index, from_arr.CvPtr);
        }
        #endregion
        #region SeqInvert
#if LANG_JP
        ...
        /// <returns></returns>
#endif
        public static int SeqPartition(CvSeq seq, CvMemStorage storage, out CvSeq labels, CvCmpFunc is_equal)
        {
            if (seq == null)
                throw new ArgumentNullException("seq");
            if (is_equal == null)
                throw new ArgumentNullException("is_equal");

            IntPtr storagePtr = (storage == null) ? IntPtr.Zero : storage.CvPtr;
            IntPtr labelsPtr = IntPtr.Zero;

            GCHandle gch = GCHandle.Alloc(is_equal);
            int result = CvInvoke.cvSeqPartition(seq.CvPtr, storagePtr, ref labelsPtr, is_equal, IntPtr.Zero);
            gch.Free();
        ...
        /// <returns></returns>
#endif
        public static int SeqPartition<T>(CvSeq<T> seq, CvMemStorage storage, out CvSeq labels, CvCmpFunc<T> is_equal)
            where T : struct
        {
            if (is_equal == null)
                throw new ArgumentNullException("is_equal");

            CvCmpFunc is_equal_nongeneric = delegate(IntPtr a, IntPtr b)
            {
                ...
                return is_equal(aValue, bValue);
            };

            return SeqPartition(seq, storage, out labels, is_equal_nongeneric);
        }
        #endregion
        #region SeqPop
#if LANG_JP
...
#endif
        public static void SeqPopMulti<T>(CvSeq seq, out T[] elements, int count, InsertPosition in_front) where T : struct
        {
            if (seq == null)
            {
                throw new ArgumentNullException("seq");
            ...
            //if (typeof(T).IsValueType)  // where T : struct
            {
                ...
                {
                    CvInvoke.cvSeqPopMulti(seq.CvPtr, elementsPtr, elements.Length, in_front);
                }
            }
            /*
        ...
        /// <param name="in_front">The flags specifying the modified sequence end</param>
#endif
        public static void SeqPushMulti<T>(CvSeq seq, T[] elements, InsertPosition in_front) where T : struct
        {
            if (seq == null)
                throw new ArgumentNullException("seq");
            if (elements == null)
            ...
            //if (typeof(T).IsValueType)  // where T : struct
            {
                using (var elementsPtr = new ArrayAddress1<T>(elements))
                {
                    CvInvoke.cvSeqPushMulti(seq.CvPtr, elementsPtr, elements.Length, in_front);
                }
            }
            /*
            else  // CvSeq, CvMat, etc.
        ...
        /// <returns></returns>
#endif
        public static IntPtr SeqSearch(CvSeq seq, IntPtr elem, CvCmpFunc func, bool is_sorted, out int elem_idx)
        {
            if (seq == null)
                throw new ArgumentNullException("seq");
            if (func == null)
            ...
            {
                return CvInvoke.cvSeqSearch(seq.CvPtr, elem, func, is_sorted, out elem_idx);
            }
        }
#if LANG_JP
        /// <summary>
        ...
        /// <returns></returns>
#endif
        public static T SeqSearch<T>(CvSeq seq, T elem, CvCmpFunc<T> func, bool is_sorted, out int elem_idx) where T : struct
        {
            if (seq == null)
                throw new ArgumentNullException("seq");
            if (func == null)
        ...
        /// <returns></returns>
#endif
        public static CvSeq SeqSlice(CvSeq seq, CvSlice slice, CvMemStorage storage, bool copy_data)
        {
            if (seq == null)
            {
                throw new ArgumentNullException("seq");
            ...
            IntPtr storagePtr = (storage == null) ? IntPtr.Zero : storage.CvPtr;
            IntPtr result = CvInvoke.cvSeqSlice(seq.CvPtr, slice, storagePtr, copy_data);
            if (result == IntPtr.Zero)
                return null;
            else
        ...
        {
            return SeqSlice<T>(seq, slice, null, false);
        }
#if LANG_JP
        /// <summary>
        /// シーケンススライスのための別のヘッダを作成する
        ...
        {
            return SeqSlice<T>(seq, slice, storage, false);
        }
#if LANG_JP
        /// <summary>
        /// シーケンススライスのための別のヘッダを作成する
        ...
        /// <returns></returns>
#endif
        public static CvSeq<T> SeqSlice<T>(CvSeq<T> seq, CvSlice slice, CvMemStorage storage, bool copy_data) where T : struct
        {
            CvSeq result = SeqSlice((CvSeq)seq, slice, storage, copy_data);
...
#endif
        public static int SetAdd(CvSet set_header)
        {
            if (set_header == null)
            {
                throw new ArgumentNullException("set_header");
            }
            IntPtr elem_ptr = IntPtr.Zero;
            IntPtr instead_elem_ptr = IntPtr.Zero;
            return CvInvoke.cvSetAdd(set_header.CvPtr, elem_ptr, ref instead_elem_ptr);
        }
#if LANG_JP
        /// <summary>
        /// セットに新しいノード（要素）を追加する
        ...
        /// <returns>the index to the node</returns>
#endif
        public static int SetAdd(CvSet set_header, CvSetElem elem)
        {
            if (set_header == null)
            {
                throw new ArgumentNullException("set_header");
            }
            IntPtr elem_ptr = (elem == null) ? IntPtr.Zero : elem.CvPtr;
            IntPtr instead_elem_ptr = IntPtr.Zero;
            return CvInvoke.cvSetAdd(set_header.CvPtr, elem_ptr, ref instead_elem_ptr);
        }
#if LANG_JP
        ...
        /// <returns>the index to the node</returns>
#endif
        public static int SetAdd(CvSet set_header, CvSetElem elem, out CvSetElem inserted_elem)
        {
            if (set_header == null)
            {
                throw new ArgumentNullException("set_header");
            ...
            IntPtr elem_ptr = (elem == null) ? IntPtr.Zero : elem.CvPtr;
            inserted_elem = new CvSetElem();
            IntPtr instead_elem_ptr = inserted_elem.CvPtr;
            return CvInvoke.cvSetAdd(set_header.CvPtr, elem_ptr, ref instead_elem_ptr);
        }
        #endregion
        #region SetBinRanges
        ...
        /// <returns></returns>
#endif
        public static int SetCaptureProperty(CvCapture capture, int property_id, double value)
        {
            if (capture == null)
            {
                throw new ArgumentNullException("capture");
            }
            return CvInvoke.cvSetCaptureProperty(capture.CvPtr, property_id, value);
        }
#if LANG_JP
        /// <summary>
        /// ビデオキャプチャのプロパティをセットする
        ...
        /// <returns></returns>
#endif
        public static int SetCaptureProperty(CvCapture capture, CaptureProperty property_id, double value)
        {
            return SetCaptureProperty(capture, (int)property_id, value);
        }
        #endregion
        #region SetData
#if LANG_JP
...
#endif
        public static void SetImagesForHaarClassifierCascade(CvHaarClassifierCascade cascade, CvArr sum, CvArr sqsum, CvArr tilted_sum, double scale)
        {
            if (cascade == null)
            {
                throw new ArgumentNullException("cascade");
            }
            IntPtr sum_ptr = (sum == null) ? IntPtr.Zero : sum.CvPtr;
            IntPtr sqsum_ptr = (sqsum == null) ? IntPtr.Zero : sqsum.CvPtr;
            IntPtr tilted_sum_ptr = (tilted_sum == null) ? IntPtr.Zero : tilted_sum.CvPtr;
            CvInvoke.cvSetImagesForHaarClassifierCascade(cascade.CvPtr, sum_ptr, sqsum_ptr, tilted_sum_ptr, scale);
        }
        #endregion
...
#endif
        public static void SetMouseCallback(string window_name, CvMouseCallback on_mouse)
        {
            if (string.IsNullOrEmpty(window_name))
                throw new ArgumentNullException("window_name");
            if (on_mouse == null)
                throw new ArgumentNullException("on_mouse");
            //CvDll.cvSetMouseCallback(window_name, on_mouse);
            CvWindow window = CvWindow.GetWindowByName(window_name);
            if (window != null)
            {
                window.MouseCallback = on_mouse;
            }
        }
        #endregion
        #region SetNew
...
#endif
        public static CvSetElem SetNew(CvSet set_header)
        {
            if (set_header == null)
            {
                throw new ArgumentNullException("set_header");
            ...
            unsafe
            {
                WCvSet* set = (WCvSet*)(set_header.CvPtr);
                ...
                {
                    CvInvoke.cvSetAdd(set_header.CvPtr, IntPtr.Zero, ref elemPtr);
                }
        ...
        /// <param name="on_postprocess"></param>
#endif
        public static void SetPostprocessFuncWin32(CvWin32WindowCallback on_postprocess)
        {
            postProcess = on_postprocess;
            CvInvoke.cvSetPostprocessFuncWin32(on_postprocess);
        }
        ...
        /// <param name="on_preprocess"></param>
#endif
        public static void SetPreprocessFuncWin32(CvWin32WindowCallback on_preprocess)
        {
            preProcess = on_preprocess;
            CvInvoke.cvSetPreprocessFuncWin32(on_preprocess);
        }
...
#endif
        public static void SetRemove(CvSet set_header, int index)
        {
            if (set_header == null)
            {
                throw new ArgumentNullException("set_header");
            }
            CvInvoke.cvSetRemove(set_header.CvPtr, index);
        }
        #endregion
...
#endif
        public static void SetRemoveByPtr(CvSet set_header, IntPtr elem)
        {
            if (set_header == null)
            {
                throw new ArgumentNullException("set_header");
            ...
            unsafe
            {
                WCvSet* _set_header = (WCvSet*)set_header.CvPtr; 
                WCvSetElem* _elem = (WCvSetElem*)elem;
                Debug.Assert(_elem->flags >= 0 /*&& (elem->flags & CV_SET_ELEM_IDX_MASK) < set_header->total*/ );
                _elem->next_free = _set_header->free_elems;
                _elem->flags = (_elem->flags & CvConst.CV_SET_ELEM_IDX_MASK) | CvConst.CV_SET_ELEM_FREE_FLAG;
                _set_header->free_elems = _elem;
                _set_header->active_count--;
            }
        }
        #endregion
...
#endif
        public static void SetSeqBlockSize(CvSeq seq, int delta_elems)
        {
            if (seq == null)
            {
                throw new ArgumentNullException("seq");
            }
            CvInvoke.cvSetSeqBlockSize(seq.CvPtr, delta_elems);
        }
        #endregion
...
#endif
        public static void SetSeqReaderPos(CvSeqReader reader, int index, bool is_relative)
        {
            if (reader == null)
            {
                throw new ArgumentNullException("reader");
            }
            CvInvoke.cvSetSeqReaderPos(reader.CvPtr, index, is_relative);
        }
        #endregion
...
#endif
        public static void SetTrackbarPos(string trackbar_name, string window_name, int pos)
        {
            if (string.IsNullOrEmpty(trackbar_name))
                throw new ArgumentNullException("trackbar_name");
            if (string.IsNullOrEmpty(window_name))
                throw new ArgumentNullException("window_name");
            CvInvoke.cvSetTrackbarPos(trackbar_name, window_name, pos);
        }
        #endregion
...
#endif
        public static void SetWindowProperty(string name, WindowProperty prop_id, double prop_value)
        {
            if (string.IsNullOrEmpty(name))
            {
                throw new ArgumentNullException("name");
            }
            CvInvoke.cvSetWindowProperty(name, prop_id, prop_value);
        }
        #endregion
        #region SetZero
#if LANG_JP
        ...
        /// <param name="win">最小値を探索する各点の近傍のサイズ． Width と Height は奇数でなければならない．</param>
        /// <param name="criteria">終了条件</param>
        /// <param name="calc_gradient">勾配フラグ．trueの場合，この関数は全ての画像ピクセルに対する勾配の強さを計算し，これをエネルギー場と見なす． falseの場合は，入力画像自体がエネルギー場と見なされる．</param>
#else
        /// <summary>
        /// Changes contour position to minimize its energy
        /// </summary>
...
#endif
        public static void SnakeImage(IplImage image, CvPoint[] points, float alpha, float beta, float gamma, CvSize win, CvTermCriteria criteria, bool calc_gradient)
        {
            if (image == null)
                throw new ArgumentNullException("image");
            if (points == null)
            ...
            {
                CvInvoke.cvSnakeImage(image.CvPtr, pointsPtr, points.Length, ref alpha, ref beta, ref gamma, CvConst.CV_VALUE, win, criteria, calc_gradient);
            }
        }
#if LANG_JP
        /// <summary>
        ...
        public static void SnakeImage(IplImage image, CvPoint[] points, float[] alpha, float[] beta, float[] gamma, CvSize win, CvTermCriteria criteria, bool calc_gradient)
        {
            ...
            using (var pointsPtr = new ArrayAddress1<CvPoint>(points))
            {
                CvInvoke.cvSnakeImage(image.CvPtr, pointsPtr, points.Length, alpha, beta, gamma, CvConst.CV_ARRAY, win, criteria, calc_gradient);
            }
        }
        #endregion
        #region Sobel
...
#endif
        public static void Sobel(CvArr src, CvArr dst, int xorder, int yorder, ApertureSize aperture_size)
        {
            if (src == null)
                throw new ArgumentNullException("src");
            if (dst == null)
                throw new ArgumentNullException("dst");
            CvInvoke.cvSobel(src.CvPtr, dst.CvPtr, xorder, yorder, aperture_size);
        }
        #endregion
        #region Solve
#if LANG_JP
        ...
        {
            return StartFindContours(image, storage, header_size, ContourRetrieval.List, ContourChain.ApproxSimple, new CvPoint());
        }
#if LANG_JP
        /// <summary>
        /// 輪郭走査処理の初期化を行う
        ...
        {
            return StartFindContours(image, storage, header_size, mode, ContourChain.ApproxSimple, new CvPoint());
        }
#if LANG_JP
        /// <summary>
        /// 輪郭走査処理の初期化を行う
        ...
        {
            return StartFindContours(image, storage, header_size, mode, method, new CvPoint());
        }
#if LANG_JP
        /// <summary>
        /// 輪郭走査処理の初期化を行う
        ...
        /// <param name="writer">Writer state; initialized by the function. </param>
#endif
        public static void StartWriteSeq(SeqType seq_flags, int header_size, int elem_size, CvMemStorage storage, out CvSeqWriter writer)
        {
            if (storage == null)
            {
                throw new ArgumentNullException("storage");
            ...
            writer = new CvSeqWriter();
            CvInvoke.cvStartWriteSeq(seq_flags, header_size, elem_size, storage.CvPtr, writer.CvPtr);
        }
        #endregion
...
#endif
        public static void StartWriteStruct(CvFileStorage fs, string name, NodeType struct_flags)
        {
            StartWriteStruct(fs, name, struct_flags, null);
        }
#if LANG_JP
        /// <summary>
        /// 新しい構造体の書き込みを開始する
...
#endif
        public static void StartWriteStruct(CvFileStorage fs, string name, NodeType struct_flags, string type_name)
        {
            if (fs == null)
                throw new ArgumentNullException("fs");
            CvInvoke.cvStartWriteStruct(fs.CvPtr, name, struct_flags, type_name);
        }
        #endregion
        ...
        /// </summary>
        /// <param name="object_points">オブジェクト点の結合行列，3xN あるいは Nx3．ここで， N は全てのビューにおける点数の合計． </param>
        /// <param name="image_points1">1 番目のカメラのビューにおける画像間対応点の結合行列， 2xN あるいは Nx2．ここで，N は全てのビューにおける点数の合計． </param>
        /// <param name="image_points2">2 番目のカメラのビューにおける画像間対応点の結合行列， 2xN あるいは Nx2．ここで，N は全てのビューにおける点数の合計． </param>
        /// <param name="npoints">各ビューに含まれる点数のベクトル，1xM あるいは Mx1． ここで，M はビューの数． </param>
        /// <param name="camera_matrix1">入出力カメラ行列 [fxk 0 cxk; 0 fyk  cyk; 0 0 1]． CV_CALIB_USE_INTRINSIC_GUESS か CV_CALIB_FIX_ASPECT_RATIO が指定された場 合は，これらの行列のいくつか（または全て）の要素が初期化されているはずである． </param>
        /// <param name="dist_coeffs1">カメラ1の歪み係数を表す入出力ベクトル， 4x1，1x4，5x1，1x5．</param>
        /// <param name="camera_matrix2">入出力カメラ行列 [fxk 0 cxk; 0 fyk  cyk; 0 0 1]． CV_CALIB_USE_INTRINSIC_GUESS か CV_CALIB_FIX_ASPECT_RATIO が指定された場 合は，これらの行列のいくつか（または全て）の要素が初期化されているはずである． </param>
        /// <param name="dist_coeffs2">カメラ2の歪み係数を表す入出力ベクトル， 4x1，1x4，5x1，1x5．</param>
        /// <param name="image_size">画像サイズ．カメラの内部パラメータ行列を初期化する際にのみ用いられる． </param>
...
#endif
        public static void StereoCalibrate(CvMat object_points, CvMat image_points1, CvMat image_points2, CvMat npoints,
                       CvMat camera_matrix1, CvMat dist_coeffs1, CvMat camera_matrix2, CvMat dist_coeffs2,
                       CvSize image_size, CvMat R, CvMat T)
        {
            StereoCalibrate(object_points, image_points1, image_points2, npoints,
                camera_matrix1, dist_coeffs1, camera_matrix2, dist_coeffs2,
                image_size, R, T, null, null, new CvTermCriteria(30, 1e-6), CalibrationFlag.FixIntrinsic);
        }
#if LANG_JP
        /// <summary>
        /// ステレオカメラのキャリブレーションを行う
                       ...
                       CvMat E)
        {
            StereoCalibrate(object_points, image_points1, image_points2, npoints,
                camera_matrix1, dist_coeffs1, camera_matrix2, dist_coeffs2,
                image_size, R, T, E, null, new CvTermCriteria(30, 1e-6), CalibrationFlag.FixIntrinsic);
        }
#if LANG_JP
        /// <summary>
        /// ステレオカメラのキャリブレーションを行う
                       ...
                       CvMat E, CvMat F)
        {
            StereoCalibrate(object_points, image_points1, image_points2, npoints,
                camera_matrix1, dist_coeffs1, camera_matrix2, dist_coeffs2,
                image_size, R, T, E, F, new CvTermCriteria(30, 1e-6), CalibrationFlag.FixIntrinsic);
        }
#if LANG_JP
        /// <summary>
        /// ステレオカメラのキャリブレーションを行う
        ...
        /// <param name="E">出力オプション：E行列．</param>
        /// <param name="F">出力オプション：F行列（基礎行列）．</param>
        /// <param name="term_crit">繰り返し最適化アルゴリズムのための終了条件．</param>
#else
        /// <summary>
        /// Calibrates stereo camera
        /// </summary>
        ...
        public static void StereoCalibrate(CvMat object_points, CvMat image_points1, CvMat image_points2, CvMat npoints,
                       CvMat camera_matrix1, CvMat dist_coeffs1, CvMat camera_matrix2, CvMat dist_coeffs2,
                       CvSize image_size, CvMat R, CvMat T,
                       CvMat E, CvMat F, CvTermCriteria term_crit)
        {
            StereoCalibrate(object_points, image_points1, image_points2, npoints,
                camera_matrix1, dist_coeffs1, camera_matrix2, dist_coeffs2,
                image_size, R, T, E, F, term_crit, CalibrationFlag.FixIntrinsic);
        }
#if LANG_JP
        /// <summary>
        /// ステレオカメラのキャリブレーションを行う
        ...
        public static void StereoCalibrate(CvMat object_points, CvMat image_points1, CvMat image_points2, CvMat npoints,
                       CvMat camera_matrix1, CvMat dist_coeffs1, CvMat camera_matrix2, CvMat dist_coeffs2,
                       CvSize image_size, CvMat R, CvMat T,
                       CvMat E, CvMat F, CvTermCriteria term_crit, CalibrationFlag flags)
        {
            if (object_points == null)
                throw new ArgumentNullException("object_points");
            if (image_points1 == null)
                throw new ArgumentNullException("image_points1");
            if (image_points2 == null)
                throw new ArgumentNullException("image_points2");
            if (npoints == null)
                throw new ArgumentNullException("npoints");
            if (camera_matrix1 == null)
                throw new ArgumentNullException("camera_matrix1");
            if (dist_coeffs1 == null)
                throw new ArgumentNullException("dist_coeffs1");
            if (camera_matrix2 == null)
                throw new ArgumentNullException("camera_matrix2");
            if (dist_coeffs2 == null)
                throw new ArgumentNullException("dist_coeffs2");

            IntPtr E_ptr = (E == null) ? IntPtr.Zero : E.CvPtr;
            IntPtr F_ptr = (F == null) ? IntPtr.Zero : F.CvPtr;

            CvInvoke.cvStereoCalibrate(object_points.CvPtr, image_points1.CvPtr, image_points2.CvPtr, npoints.CvPtr,
                camera_matrix1.CvPtr, dist_coeffs1.CvPtr, camera_matrix2.CvPtr, dist_coeffs2.CvPtr,
                image_size, R.CvPtr, T.CvPtr, E_ptr, F_ptr, term_crit, flags);
        }
        #endregion
        ...
        /// ステレオカメラの平行化変換を求める
        /// </summary>
        /// <param name="camera_matrix1">カメラ行列 [fxk 0 cxk; 0 fyk  cyk; 0 0 1]．</param>
        /// <param name="camera_matrix2">カメラ行列 [fxk 0 cxk; 0 fyk  cyk; 0 0 1]．</param>
        /// <param name="dist_coeffs1">カメラ1の歪み係数ベクトル．4x1，1x4，5x1，1x5．</param>
        /// <param name="dist_coeffs2">カメラ2の歪み係数ベクトル．4x1，1x4，5x1，1x5．</param>
        /// <param name="image_size">ステレオカメラのキャリブレーションに用いられる画像サイズ． </param>
        /// <param name="R">1 番目と2 番目のカメラの座標系間の回転行列． </param>
        /// <param name="T">カメラ座標系間の並進ベクトル． </param>
...
#endif
        public static void StereoRectify(CvMat camera_matrix1, CvMat camera_matrix2, CvMat dist_coeffs1, CvMat dist_coeffs2,
                             CvSize image_size, CvMat R, CvMat T, CvMat R1, CvMat R2, CvMat P1, CvMat P2)
        {
            StereoRectify(camera_matrix1, camera_matrix2, dist_coeffs1, dist_coeffs2,
                image_size, R, T, R1, R2, P1, P2, null, StereoRectificationFlag.ZeroDisparity);
        }
#if LANG_JP
        /// <summary>
        /// ステレオカメラの平行化変換を求める
        ...
        {
            StereoRectify(camera_matrix1, camera_matrix2, dist_coeffs1, dist_coeffs2,
                image_size, R, T, R1, R2, P1, P2, Q, StereoRectificationFlag.ZeroDisparity);
        }
#if LANG_JP
        /// <summary>
        /// ステレオカメラの平行化変換を求める
        ...
        {
            CvRect valid_pix_ROI1, valid_pix_ROI2;
            StereoRectify(camera_matrix1, camera_matrix2, dist_coeffs1, dist_coeffs2,
                image_size, R, T, R1, R2, P1, P2, Q, flags, 
                -1, new CvSize(0, 0), out valid_pix_ROI1, out valid_pix_ROI2);
        }
#if LANG_JP
        /// <summary>
        /// ステレオカメラの平行化変換を求める
                             ...
                             double alpha)
        {
            ...
            StereoRectify(camera_matrix1, camera_matrix2, dist_coeffs1, dist_coeffs2,
                image_size, R, T, R1, R2, P1, P2, Q, flags,
                alpha, new CvSize(0,0), out valid_pix_ROI1, out valid_pix_ROI2);
        }
#if LANG_JP
        /// <summary>
        /// ステレオカメラの平行化変換を求める
        ...
        public static void StereoRectify(CvMat camera_matrix1, CvMat camera_matrix2, CvMat dist_coeffs1, CvMat dist_coeffs2,
                             CvSize image_size, CvMat R, CvMat T, CvMat R1, CvMat R2, CvMat P1, CvMat P2,
                             CvMat Q, StereoRectificationFlag flags, 
                             double alpha, CvSize new_image_size)
        {
            CvRect valid_pix_ROI1, valid_pix_ROI2;
            StereoRectify(camera_matrix1, camera_matrix2, dist_coeffs1, dist_coeffs2,
                image_size, R, T, R1, R2, P1, P2, Q, flags, 
                alpha, new_image_size, out valid_pix_ROI1, out valid_pix_ROI2);
        }
                             ...
                             double alpha, CvSize new_image_size, out CvRect valid_pix_ROI1, out CvRect valid_pix_ROI2)
        {
            ...
            IntPtr Q_ptr = (Q == null) ? IntPtr.Zero : Q.CvPtr;
            CvInvoke.cvStereoRectify(camera_matrix1.CvPtr, camera_matrix2.CvPtr, dist_coeffs1.CvPtr, dist_coeffs2.CvPtr,
                image_size, R.CvPtr, T.CvPtr, R1.CvPtr, R2.CvPtr, P1.CvPtr, P2.CvPtr, Q_ptr, flags, 
                alpha, new_image_size, out valid_pix_ROI1, out valid_pix_ROI2);
        }
        #endregion
        ...
        /// <param name="img_size">画像サイズ．</param>
        ...
        /// <returns></returns>
#endif
        public static int StereoRectifyUncalibrated(CvMat points1, CvMat points2, CvMat F, CvSize img_size, CvMat H1, CvMat H2)
        {
            return StereoRectifyUncalibrated(points1, points2, F, img_size, H1, H2, 5);
        }
#if LANG_JP
        /// <summary>
        /// キャリブレーションされていないステレオカメラの平行化変換を求める
        ...
        /// <returns></returns>
#endif
        public static int StereoRectifyUncalibrated(CvMat points1, CvMat points2, CvMat F, CvSize img_size, CvMat H1, CvMat H2, double threshold)
        {
            if (points1 == null)
            ...
            return CvInvoke.cvStereoRectifyUncalibrated(points1.CvPtr, points2.CvPtr, F.CvPtr, img_size, H1.CvPtr, H2.CvPtr, threshold);
        }
        #endregion
        #region Sub
#if LANG_JP
        ...
        public static CvSubdiv2DEdge Subdiv2DGetEdge(CvSubdiv2DEdge edge, CvNextEdgeType type)
        {
            unsafe
            {
                ulong _edge = (ulong)edge;
                WCvQuadEdge2D* e = (WCvQuadEdge2D*)((long)_edge & ~3L);
                _edge = (ulong)e->next(
                    ((long)_edge + (long)type) & 3
                ).ToInt64();
                return new CvSubdiv2DEdge(
                    (ulong)(((long)_edge & ~3L) + (((long)_edge + ((long)type >> 4)) & 3))
                );
            }
        }
        #endregion
        ...
        public static CvSubdiv2DEdge Subdiv2DRotateEdge(CvSubdiv2DEdge edge, RotateEdgeFlag rotate)
        {
            unsafe
            {
                long _edge = (long)(ulong)edge;
                return new CvSubdiv2DEdge(
                    (ulong)((_edge & ~3L) + ((_edge + (long)rotate) & 3))
                );
            }
        }
        #endregion
...
#endif
        public static void SubstituteContour(CvContourScanner scanner, CvSeq<CvPoint> new_contour)
        {
            if (scanner == null)
                throw new ArgumentNullException("scanner");
            if (new_contour == null)
                throw new ArgumentNullException("new_contour");
            CvInvoke.cvSubstituteContour(scanner.CvPtr, new_contour.CvPtr);
        }
        #endregion
        #region SURFParams
#if LANG_JP
