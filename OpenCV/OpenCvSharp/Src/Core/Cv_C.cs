...
namespace OpenCvSharp
{
    public static partial class Cv
    {
        ...
        /// <param name="flags"></param>
#endif
        public static void CalcAffineFlowPyrLK( CvArr prev, CvArr curr, CvArr  prev_pyr, CvArr curr_pyr,
            CvPoint2D32f[] prev_features, out CvPoint2D32f[] curr_features, out float[] matrices, int count,
            CvSize win_size, int level, out sbyte[] status, out float[] track_error, CvTermCriteria criteria, LKFlowFlag flags )
        {
            if (prev == null)
            ...
            if (prev_features == null)
                throw new ArgumentNullException("prev_features");
            
            curr_features = new CvPoint2D32f[prev_features.Length];
            matrices = new float[prev_features.Length];
            status = new sbyte[prev_features.Length];
            track_error = new float[prev_features.Length];

            CvInvoke.cvCalcAffineFlowPyrLK(prev.CvPtr, curr.CvPtr, prev_pyr.CvPtr, curr_pyr.CvPtr, prev_features, curr_features, matrices,
                prev_features.Length, win_size, level, status, track_error, criteria, flags);
        }
        #endregion
        #region CalcArrHist
#if LANG_JP
        ...
        /// <returns></returns>
#endif
        public static void CalcArrBackProject(CvArr[] image, CvArr back_project, CvHistogram hist)
        {
            if (image == null)
            {
                throw new ArgumentNullException("image");
            }
            if (back_project == null)
            {
                throw new ArgumentNullException("back_project");
            }
            if (hist == null)
            {
                throw new ArgumentNullException("hist");
            ...
            CvInvoke.cvCalcArrBackProject(image_ptr, back_project.CvPtr, hist.CvPtr);
        }
#if LANG_JP
        /// <summary>
        /// バックプロジェクションの計算を行う
        ...
        /// <returns></returns>
#endif
        public static void CalcBackProject(IplImage[] image, CvArr back_project, CvHistogram hist)
        {
            CalcArrBackProject(image, back_project, hist);
        }
        #endregion
        ...
        /// <returns></returns>
#endif
        public static float CalcEMD2(CvArr signature1, CvArr signature2, DistanceType distance_type)
        {
            float lower_bound = 0;
            return CalcEMD2(signature1, signature2, distance_type, null, null, null, ref lower_bound);
        }
#if LANG_JP
        /// <summary>
        /// 2つの重み付き点分布間の "最小コスト（minimal work）" 距離を計算する．
        ...
        /// <returns></returns>
#endif
        public static float CalcEMD2(CvArr signature1, CvArr signature2, DistanceType distance_type, CvDistanceFunction distance_func)
        {
            float lower_bound = 0;
            return CalcEMD2(signature1, signature2, distance_type, distance_func, null, null, ref lower_bound);
        }
#if LANG_JP
        /// <summary>
        /// 2つの重み付き点分布間の "最小コスト（minimal work）" 距離を計算する．
        ...
        /// <returns></returns>
#endif
        public static float CalcEMD2(CvArr signature1, CvArr signature2, DistanceType distance_type, CvDistanceFunction distance_func, CvArr cost_matrix)
        {
            float lower_bound = 0;
            return CalcEMD2(signature1, signature2, distance_type, distance_func, cost_matrix, null, ref lower_bound);
        }
#if LANG_JP
        /// <summary>
        /// 2つの重み付き点分布間の "最小コスト（minimal work）" 距離を計算する．
        ...
        /// <returns></returns>
#endif
        public static float CalcEMD2(CvArr signature1, CvArr signature2, DistanceType distance_type, CvDistanceFunction distance_func, CvArr cost_matrix, CvArr flow)
        {
            float lower_bound = 0;
            return CalcEMD2(signature1, signature2, distance_type, distance_func, cost_matrix, flow, ref lower_bound);
        }
#if LANG_JP
        /// <summary>
        /// 2つの重み付き点分布間の "最小コスト（minimal work）" 距離を計算する．
        ...
        /// <returns></returns>
#endif
        public static float CalcEMD2(CvArr signature1, CvArr signature2, DistanceType distance_type, CvDistanceFunction distance_func, CvArr cost_matrix, CvArr flow, ref float lower_bound)
        {
            if (signature1 == null)
                throw new ArgumentNullException("signature1");
            if (signature2 == null)
                throw new ArgumentNullException("signature2");
            IntPtr cost_matrix_ptr = (cost_matrix == null) ? IntPtr.Zero : cost_matrix.CvPtr;
            IntPtr flow_ptr = (cost_matrix == null) ? IntPtr.Zero : flow.CvPtr;
            using (ScopedGCHandle funcHandle = new ScopedGCHandle(distance_func, GCHandleType.Normal))
            {
                return CvInvoke.cvCalcEMD2(signature1.CvPtr, signature2.CvPtr, distance_type, distance_func, cost_matrix_ptr, flow_ptr, ref lower_bound, 0);
            }
        }
        #endregion
...
#endif
        public static void CalcMotionGradient(CvArr mhi, CvArr mask, CvArr orientation, double delta1, double delta2, ApertureSize aperture_size)
        {
            if (mhi == null)
                throw new ArgumentNullException("mhi");
            if (orientation == null)
            ...
            CvInvoke.cvCalcMotionGradient(mhi.CvPtr, mask.CvPtr, orientation.CvPtr, delta1, delta2, aperture_size);
        }
        #endregion
...
#endif
        public static void CalcOpticalFlowBM(CvArr prev, CvArr curr, CvSize block_size, CvSize shift_size, CvSize max_range, bool use_previous, CvArr velx, CvArr vely)
        {
            if (prev == null)
                throw new ArgumentNullException("prev");
            if (curr == null)
            ...
            CvInvoke.cvCalcOpticalFlowBM(prev.CvPtr, curr.CvPtr, block_size, shift_size, max_range, use_previous, velx.CvPtr, vely.CvPtr);
        }
        #endregion
        ...
        /// <param name="vely">Vertical component of the optical flow of the same size as input images, 32-bit floating-point, single-channel. </param>
#endif
        public static void CalcOpticalFlowLK(CvArr prev, CvArr curr, CvSize win_size, CvArr velx, CvArr vely)
        {
            if (prev == null)
                throw new ArgumentNullException("prev");
            if (curr == null)
            ...
            if (vely == null)
                throw new ArgumentNullException("vely");
            CvInvoke.cvCalcOpticalFlowLK(prev.CvPtr, curr.CvPtr, win_size, velx.CvPtr, vely.CvPtr);
        }
        #endregion
...
#endif
        public static void CalcOpticalFlowPyrLK(CvArr prev, CvArr curr, CvArr prev_pyr, CvArr curr_pyr, CvPoint2D32f[] prev_features, out CvPoint2D32f[] curr_features,
            CvSize win_size, int level, out sbyte[] status, CvTermCriteria criteria, LKFlowFlag flags)
        {
            float[] track_error;
            CalcOpticalFlowPyrLK(prev, curr, prev_pyr, curr_pyr, prev_features, out curr_features, win_size, level, out status, out track_error, criteria, flags);
        }
#if LANG_JP
        /// <summary>
        ///  Lucas-Kanade オプティカルフローの， ピラミッドを用いて疎な特徴に対応した反復バージョン（[Bouguet00]）の実装である．
            ...
            CvSize win_size, int level, out sbyte[] status, out float[] track_error, CvTermCriteria criteria, LKFlowFlag flags)
        {
            ...
            IntPtr prev_pyr_ptr = (prev_pyr == null) ? IntPtr.Zero : prev_pyr.CvPtr;
            IntPtr curr_pyr_ptr = (curr_pyr == null) ? IntPtr.Zero : curr_pyr.CvPtr;
            curr_features = new CvPoint2D32f[prev_features.Length];
            status = new sbyte[prev_features.Length];
            track_error = new float[prev_features.Length];

            CvInvoke.cvCalcOpticalFlowPyrLK(prev.CvPtr, curr.CvPtr, prev_pyr_ptr, curr_pyr_ptr, prev_features, curr_features, prev_features.Length, win_size, level, status, track_error, criteria, flags);
        }
        #endregion
        #region CalcPCA
#if LANG_JP
        ...
        {
            CalcProbDensity(hist1, hist2, dst_hist, 255);
        }
#if LANG_JP
        /// <summary>
        /// 一つのヒストグラムをもう一方のヒストグラムで割る
        ...
        public static void CalcProbDensity(CvHistogram hist1, CvHistogram hist2, CvHistogram dst_hist, double scale)
        {
            ...
            if (dst_hist == null)
                throw new ArgumentNullException("dst_hist");
            CvInvoke.cvCalcProbDensity(hist1.CvPtr, hist2.CvPtr, dst_hist.CvPtr, scale);
        }
        #endregion
        ...
        public static void CartToPolar(CvArr x, CvArr y, CvArr magnitude, CvArr angle, AngleUnit unit)
        {
            ...
            IntPtr magnitude_ptr = (magnitude == null) ? IntPtr.Zero : magnitude.CvPtr;
            IntPtr angle_ptr = (angle == null) ? IntPtr.Zero : angle.CvPtr;
            CvInvoke.cvCartToPolar(x.CvPtr, y.CvPtr, magnitude_ptr, angle_ptr, unit);
        }
        #endregion
        #region Cbrt
#if LANG_JP
        ...
        /// <returns>returns nonzero if the check succeeded, i.e. all elements are valid and within the range, and zero otherwise.</returns>
#endif
        public static bool CheckArr(CvArr arr, CheckArrFlag flags, double min_val, double max_val)
        {
            if (arr == null)
            {
                throw new ArgumentNullException("arr");
            }
            return CvInvoke.cvCheckArr(arr.CvPtr, flags, min_val, max_val) != 0;
        }
#if LANG_JP
        /// <summary>
        ...
        /// <returns>returns nonzero if the check succeeded, i.e. all elements are valid and within the range, and zero otherwise.</returns>
#endif
        public static bool CheckArray(CvArr arr, CheckArrFlag flags, double min_val, double max_val)
        {
            return CheckArr(arr, flags, min_val, max_val);
        }
        #endregion
        ...
        /// <returns></returns>
#endif
        public static CvTermCriteria CheckTermCriteria(CvTermCriteria criteria, double default_eps, int default_max_iters)
        {
            return CvInvoke.cvCheckTermCriteria(criteria, default_eps, default_max_iters);
        }
        #endregion
        #region Circle
#if LANG_JP
...
#endif
        public static int ClipLine(CvSize img_size, ref CvPoint pt1, ref CvPoint pt2)
        {
            return CvInvoke.cvClipLine(img_size, ref pt1, ref pt2);
        }
        #endregion
        #region Clone
#if LANG_JP
        ...
        /// <returns></returns>
#endif
        public static IntPtr Clone(IntPtr struct_ptr)
        {
            return CvInvoke.cvClone(struct_ptr);
        }
#if LANG_JP
        /// <summary>
        /// オブジェクトのコピーを作成する
        ...
        /// <returns></returns>
#endif
        public static T Clone<T>(T struct_ptr)
            where T : ICvPtrHolder
        {
            ...
            IntPtr result = CvInvoke.cvClone(struct_ptr.CvPtr);
            try
            {
...
#endif
        public static void Cmp(CvArr src1, CvArr src2, CvArr dst, ArrComparison cmp_op)
        {
            if (src1 == null)
            ...
            if (dst == null)
                throw new ArgumentNullException("dst");
            CvInvoke.cvCmp(src1.CvPtr, src2.CvPtr, dst.CvPtr, cmp_op);
        }
        #endregion
        #region CmpS
#if LANG_JP
        ...
        /// <param name="cmp_op">The flag specifying the relation between the elements to be checked</param>
#endif
        public static void CmpS(CvArr src, double value, CvArr dst, ArrComparison cmp_op)
        {
            if (src == null)
                throw new ArgumentNullException("src");
            if (dst == null)
                throw new ArgumentNullException("dst");
            CvInvoke.cvCmpS(src.CvPtr, value, dst.CvPtr, cmp_op);
        }
        #endregion
...
#endif
        public static void ConDensInitSampleSet(CvConDensation condens, CvMat lower_bound, CvMat upper_bound)
        {
            CvInvoke.cvConDensInitSampleSet(condens.CvPtr, lower_bound.CvPtr, upper_bound.CvPtr);
        }
        #endregion
        ...
        public static void CopyHist(CvHistogram src, ref CvHistogram dst)
        {
            ...
            if (dst == null)
            {
                ...
                CvInvoke.cvCopyHist(src.CvPtr, ref dst_ptr);
                dst = new CvHistogram(dst_ptr);
            }
            else
            {
                IntPtr dst_ptr = dst.CvPtr;
                CvInvoke.cvCopyHist(src.CvPtr, ref dst_ptr);
            }
        }
        #endregion
...
#endif
        public static CvConDensation CreateConDensation(int dynam_params, int measure_params, int sample_count)
        {
            return new CvConDensation(dynam_params, measure_params, sample_count);
        }
        #endregion
        ...
        public static CvGraphScanner CreateGraphScanner(CvGraph graph, CvGraphVtx vtx, GraphScannerMask mask)
        {
            ...
            return new CvGraphScanner(CvInvoke.cvCreateGraphScanner(graph.CvPtr, vtx_ptr, mask));
        }
        #endregion
        #region CreateImage
#if LANG_JP
        ...
        /// <returns></returns>
#endif
        public static CvMemStorage CreateMemStorage(int block_size)
        {
            IntPtr ptr = CvInvoke.cvCreateMemStorage(block_size);
            if (ptr == IntPtr.Zero)
                return null;
            else
                return new CvMemStorage(ptr);
        ...
        /// <returns></returns>
#endif
        public static CvPOSITObject CreatePOSITObject(CvPoint3D32f[] points, int point_count)
        {
            if (points == null)
            {
                throw new ArgumentNullException("points");
            }
            IntPtr ptr = CvInvoke.cvCreatePOSITObject(points, point_count);
            if (ptr == IntPtr.Zero)
                return null;
            else
        ...
        /// <returns></returns>
#endif
        public static CvMat[] CreatePyramid(CvArr img, int extra_layers, double rate, CvSize[] layer_sizes, CvArr bufarr, bool calc, CvFilter filter)
        {
            if (img == null)
                throw new ArgumentNullException("img");
            if (extra_layers < 0)
                throw new ArgumentOutOfRangeException("extra_layers", "The number of extra layers must be non negative");

            IntPtr bufarrPtr = (bufarr == null) ? IntPtr.Zero : bufarr.CvPtr;

            IntPtr result = CvInvoke.cvCreatePyramid(img.CvPtr, extra_layers, rate, layer_sizes, bufarrPtr, calc, filter);
            if (result == IntPtr.Zero)
            {
                return null;
            }
        ...
        {
            return CreateSpillTree(raw_data, 50, .7, .1);
        }
#if LANG_JP
        /// <summary>
        /// 
        ...
        {
            return CreateSpillTree(raw_data, naive, .7, .1);
        }
#if LANG_JP
        /// <summary>
        /// 
        ...
        {
            return CreateSpillTree(raw_data, naive, rho, .1);
        }
#if LANG_JP
        /// <summary>
        /// 
        ...
        public static CvFeatureTree CreateSpillTree(CvMat raw_data, int naive, double rho, double tau)
        {
            if (raw_data == null)
            {
                throw new ArgumentNullException("raw_data");
            }
            IntPtr ptr = CvInvoke.cvCreateSpillTree(raw_data.CvPtr, naive, rho, tau);
            if (ptr == IntPtr.Zero)
                return null;
            else
                return new CvFeatureTree(ptr);
        ...
        /// <returns></returns>
#endif
        public static CvSeq CreateSeq(SeqType seq_flags, int header_size, int elem_size, CvMemStorage storage)
        {
            if (storage == null)
            {
                throw new ArgumentNullException("storage");
            }
            IntPtr ptr = CvInvoke.cvCreateSeq(seq_flags, header_size, elem_size, storage.CvPtr);
            if (ptr == IntPtr.Zero)
                return null;
            else
        ...
        /// <returns></returns>
#endif
        public static CvSeq<T> CreateSeq<T>(SeqType seq_flags, int header_size, CvMemStorage storage) where T : struct
        {
            CvSeq seq = CreateSeq(seq_flags, header_size, Marshal.SizeOf(typeof(T)), storage);
            if (seq == null)
                return null;
            else
        ...
        /// <returns></returns>
#endif
        public static CvSet CreateSet(SeqType set_flags, int header_size, int elem_size, CvMemStorage storage)
        {
            if (storage == null)
            {
                throw new ArgumentNullException("storage");
            }
            IntPtr result = CvInvoke.cvCreateSet(set_flags, header_size, elem_size, storage.CvPtr);
            if (result == IntPtr.Zero)
                return null;
            else
        ...
        /// <returns></returns>
#endif
        public static CvSet<T> CreateSet<T>(SeqType set_flags, int header_size, int elem_size, CvMemStorage storage) where T : struct
        {
            if (storage == null)
            {
                throw new ArgumentNullException("storage");
        ...
        /// <returns></returns>
#endif
        public static IplConvKernel CreateStructuringElementEx(int cols, int rows, int anchor_x, int anchor_y, ElementShape shape)
        {
            return new IplConvKernel(cols, rows, anchor_x, anchor_y, shape);
        }
#if LANG_JP
        /// <summary>
        /// 膨張・収縮処理に用いる構造要素を生成する
        ...
        {
            return new IplConvKernel(cols, rows, anchor_x, anchor_y, shape, values);
        }
        #endregion
        ...
        /// <returns></returns>
#endif
        public static CvSubdiv2D CreateSubdiv2D(SeqType subdiv_type, Int32 header_size, Int32 vtx_size, Int32 quadedge_size, CvMemStorage storage)
        {
            if (storage == null)
            {
                throw new ArgumentNullException("storage");
            }
            IntPtr result = CvInvoke.cvCreateSubdiv2D(subdiv_type, header_size, vtx_size, quadedge_size, storage.CvPtr);
            if (result == IntPtr.Zero)
                return null;
            else
        ...
        {
            return CvtSeqToArray<T>(seq, out elements, CvSlice.WholeSeq);
        }
#if LANG_JP
        /// <summary>
        /// シーケンスをメモリ内の連続した一つのブロックにコピーする
