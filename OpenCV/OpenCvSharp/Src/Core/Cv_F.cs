...
namespace OpenCvSharp
{
    public static partial class Cv
    {
...
#endif
        public static void FillConvexPoly(CvArr img, CvPoint[] pts, CvScalar color, LineType line_type)
        {
            FillConvexPoly(img, pts, color, line_type, 0);
        }
#if LANG_JP
        /// <summary>
        /// 凸ポリゴンの内部を塗りつぶす．
...
#endif
        public static void FillConvexPoly(CvArr img, CvPoint[] pts, CvScalar color, LineType line_type, int shift)
        {
            if (img == null)
            ...
            CvInvoke.cvFillConvexPoly(img.CvPtr, pts, pts.Length, color, line_type, shift);
        }
        #endregion
        #region FillPoly
#if LANG_JP
...
#endif
        public static void FillPoly(CvArr img, CvPoint[][] pts, CvScalar color, LineType line_type)
        {
            FillPoly(img, pts, color, line_type, 0);
        }
#if LANG_JP
        /// <summary>
        /// ポリゴン内部を塗りつぶす
        ...
        /// <param name="shift">Number of fractional bits in the vertex coordinates. </param>
#endif
        public static void FillPoly(CvArr img, CvPoint[][] pts, CvScalar color, LineType line_type, int shift)
        {
            if (img == null)
                throw new ArgumentNullException("img");
            if (pts == null)
            ...
            {
                CvInvoke.cvFillPoly(img.CvPtr, ptsPtr.Pointer, npts, contours, color, line_type, shift);
            }
        }
        #endregion
        #region Filter2D
...
#endif
        public static bool FindChessboardCorners(CvArr image, CvSize pattern_size, out CvPoint2D32f[] corners)
        {
            int corner_count;
            return FindChessboardCorners(image, pattern_size, out corners, out corner_count, ChessboardFlag.AdaptiveThresh | ChessboardFlag.NormalizeImage);
        }
#if LANG_JP
        /// <summary>
        /// 入力画像がチェスボードパターンであるかどうかを確認し，チェスボードの各コーナーの位置検出を試みる．
        ...
        /// <returns>returns true if all the corners have been found and they have been placed in a certain order (row by row, left to right in every row), otherwise, if the function fails to find all the corners or reorder them, it returns false. </returns>
#endif
        public static bool FindChessboardCorners(CvArr image, CvSize pattern_size, out CvPoint2D32f[] corners, out int corner_count)
        {
            return FindChessboardCorners(image, pattern_size, out corners, out corner_count, ChessboardFlag.AdaptiveThresh | ChessboardFlag.NormalizeImage);
        }
#if LANG_JP
        ...
        /// <returns>returns true if all the corners have been found and they have been placed in a certain order (row by row, left to right in every row), otherwise, if the function fails to find all the corners or reorder them, it returns false. </returns>
#endif
        public static bool FindChessboardCorners(CvArr image, CvSize pattern_size, out CvPoint2D32f[] corners, out int corner_count, ChessboardFlag flags)
        {
            if (image == null)
            {
            ...
            corner_count = pattern_size.Width * pattern_size.Height;
            corners = new CvPoint2D32f[corner_count];

            using (var cornersPtr = new ArrayAddress1<CvPoint2D32f>(corners))
            {
                int result = CvInvoke.cvFindChessboardCorners(image.CvPtr, pattern_size, cornersPtr, ref corner_count, flags);
                return result != 0;
            }
        }
        #endregion
...
#endif
        public static int FindContours(CvArr image, CvMemStorage storage, out CvSeq<CvPoint> first_contour)
        {
            return FindContours(image, storage, out first_contour, CvContour.SizeOf, ContourRetrieval.List, ContourChain.ApproxSimple, new CvPoint(0, 0));
        }
#if LANG_JP
        /// <summary>
        /// 2値画像中の輪郭を見つける
        ...
        /// <returns>The number of retrieved contours.</returns>
#endif
        public static int FindContours(CvArr image, CvMemStorage storage, out CvSeq<CvPoint> first_contour, int header_size)
        {
            return FindContours(image, storage, out first_contour, header_size, ContourRetrieval.List, ContourChain.ApproxSimple, new CvPoint(0, 0));
        }
#if LANG_JP
        /// <summary>
        /// 2値画像中の輪郭を見つける
        ...
        {
            return FindContours(image, storage, out first_contour, header_size, mode, method, new CvPoint(0, 0));
        }
#if LANG_JP
        /// <summary>
        /// 2値画像中の輪郭を見つける
        ...
        public static int FindContours(CvArr image, CvMemStorage storage, out CvSeq<CvPoint> first_contour, int header_size, ContourRetrieval mode, ContourChain method, CvPoint offset)
        {
            ...
            IntPtr first_contour_ptr = IntPtr.Zero;
            int result = CvInvoke.cvFindContours(image.CvPtr, storage.CvPtr, ref first_contour_ptr, header_size, mode, method, offset);

            if (first_contour_ptr == IntPtr.Zero)
            {
                first_contour = null;
            }
            else if (method == ContourChain.Code)
            {
                first_contour = new CvChain(first_contour_ptr);
            }
            else
            {
                first_contour = new CvContour(first_contour_ptr);
            }
            return result;
        }
        #endregion
...
#endif
        public static void FindCornerSubPix(CvArr image, CvPoint2D32f[] corners, int count, CvSize win, CvSize zero_zone, CvTermCriteria criteria)
        {
            if (image == null)
                throw new ArgumentNullException("image");
            if (corners == null)
            ...
            CvInvoke.cvFindCornerSubPix(image.CvPtr, corners, count, win, zero_zone, criteria);
        }
        #endregion
...
#endif
        public static void FindExtrinsicCameraParams2(CvMat object_points, CvMat image_points, CvMat intrinsic_matrix, CvMat distortion_coeffs, CvMat rotation_vector, CvMat translation_vector)
        {
            FindExtrinsicCameraParams2(object_points, image_points, intrinsic_matrix, distortion_coeffs, rotation_vector, translation_vector, false);
        }
#if LANG_JP
        /// <summary>
        /// 既知の内部パラメータを用いて，それぞれのビューにおける外部パラメータを推定する．
        ...
        /// <param name="use_extrinsic_guess"></param>
#endif
        public static void FindExtrinsicCameraParams2(CvMat object_points, CvMat image_points, CvMat intrinsic_matrix, CvMat distortion_coeffs, CvMat rotation_vector, CvMat translation_vector, bool use_extrinsic_guess)
        {
            ...
            if (rotation_vector == null)
                throw new ArgumentNullException("rotation_vector");
            if (translation_vector == null)
                throw new ArgumentNullException("translation_vector");
            IntPtr distortion_coeffs_ptr = ToPtr(distortion_coeffs);
            CvInvoke.cvFindExtrinsicCameraParams2(object_points.CvPtr, image_points.CvPtr, intrinsic_matrix.CvPtr, distortion_coeffs_ptr, rotation_vector.CvPtr, translation_vector.CvPtr, use_extrinsic_guess);
        }
...
#endif
        public static void FindExtrinsicCameraParams2_(CvMat objectPoints, CvMat imagePoints, CvMat cameraMatrix, CvMat distCoeffs, CvMat rvec, CvMat tvec)
        {
            FindExtrinsicCameraParams2_(objectPoints, imagePoints, cameraMatrix, distCoeffs, rvec, tvec, false);
        }
#if LANG_JP
        /// <summary>
        /// 既知の内部パラメータを用いて，それぞれのビューにおける外部パラメータを推定する．
        ...
        /// <param name="useExtrinsicGuess"></param>
#endif
        public static void FindExtrinsicCameraParams2_(CvMat objectPoints, CvMat imagePoints, CvMat cameraMatrix, CvMat distCoeffs, CvMat rvec, CvMat tvec, bool useExtrinsicGuess)
        {
            ...
            unsafe
            {
                ...
                else
                {
                    ...
                    if (W[2] / W[1] < 1e-3 || count < 4)
                    {
                        ...
                        _h2.DataDouble += 1;
                        _h3.DataDouble += 2;
                        h1_norm = Math.Sqrt(h[0] * h[0] + h[3] * h[3] + h[6] * h[6]);
                        h2_norm = Math.Sqrt(h[1] * h[1] + h[4] * h[4] + h[7] * h[7]);
                    ...
                    else
                    {
                        ...
                        Cv.MulTransposed(matL, _LL, true);
                        Cv.SVD(_LL, _LW, null, _LV, SVDFlag.ModifyA | SVDFlag.V_T);
                        double[] LV12 = new double[12];
                        Array.Copy(LV, 11 * 12, LV12, 0, 12);
                        _RRt = new CvMat(3, 4, MatrixType.F64C1, LV12);
                        Cv.GetCols(_RRt, out _RR, 0, 3);
                        Cv.GetCol(_RRt, out _tt, 3);
                        if (Cv.Det(_RR) < 0)
                            Cv.Scale(_RRt, _RRt, -1);
                        double sc = Cv.Norm(_RR);
                        Cv.SVD(_RR, matW, matU, matV, SVDFlag.ModifyA | SVDFlag.U_T | SVDFlag.V_T);
                        Cv.GEMM(matU, matV, 1, null, 0, matR, GemmOperation.A_T);
                        Cv.Scale(_tt, _t, Cv.Norm(matR) / sc);
                        Cv.Rodrigues2_(matR, _r);
                    }
                }
                ...
                CvLevMarq solver = new CvLevMarq(6, count * 2, new CvTermCriteria(max_iter, float.Epsilon), true);
                ...
                for (; ; )
                {
                    ...
                    {
                        Cv.ProjectPoints2(matM, _r, _t, matA, distCoeffs, _err, null, null, null, null, null);
                    }
                    Sub(_err, _m, _err);
        ...
        internal unsafe static void UndistortPoints_(CvMat _src, CvMat _dst, CvMat _cameraMatrix, CvMat _distCoeffs, CvMat matR, CvMat matP)
        {
            ...
            */
            ...
            if (_distCoeffs != null)
            {
                ...
                _Dk = new CvMat(_distCoeffs.Rows, _distCoeffs.Cols, (MatrixType)Cv.MAKETYPE(CvConst.CV_64F, Cv.MAT_CN(_distCoeffs.Type)), k);
            ...
            if (matR != null)
            {
                if (!(Cv.IS_MAT(matR) && matR.Rows == 3 && matR.Cols == 3))
                {
                    throw new ArgumentException();
                }
                Cv.Convert(matR, _RR);
            ...
            if (matP != null)
            {
                ...
                if (!(Cv.IS_MAT(matP) && matP.Rows == 3 && (matP.Cols == 3 || matP.Cols == 4)))
                {
                }
                Cv.GetCols(matP, out _P3x3, 0, 3);
                Cv.Convert(_P3x3, _PP);
                Cv.MatMul(_PP, _RR, _RR);
            }
            ...
            int stype = Cv.MAT_TYPE(_src.Type);
            int dtype = Cv.MAT_TYPE(_dst.Type);
            int sstep = _src.Rows == 1 ? 1 : _src.Step / Cv.ELEM_SIZE(stype);
            int dstep = _dst.Rows == 1 ? 1 : _dst.Step / Cv.ELEM_SIZE(dtype);
...
#endif
        public static int FindFeaturesBoxed(CvFeatureTree tr, CvMat bounds_min, CvMat bounds_max, CvMat result)
        {
            if (tr == null)
                throw new ArgumentNullException("tr");
            if (bounds_min == null)
                throw new ArgumentNullException("bounds_min");
            if (bounds_max == null)
                throw new ArgumentNullException("bounds_max");
            if (result == null)
                throw new ArgumentNullException("result");
            return CvInvoke.cvFindFeaturesBoxed(tr.CvPtr, bounds_min.CvPtr, bounds_max.CvPtr, result.CvPtr);
        }
        #endregion
        ...
        /// <returns></returns>
#endif
        public static int FindFundamentalMat(CvMat points1, CvMat points2, CvMat fundamental_matrix)
        {
            return FindFundamentalMat(points1, points2, fundamental_matrix, FundamentalMatMethod.Ransac, 3.0, 0.99, null);
        }
#if LANG_JP
        /// <summary>
        /// 2枚の画像間の点対応から基礎行列（F行列）を計算する
        ...
        /// <returns></returns>
#endif
        public static int FindFundamentalMat(CvMat points1, CvMat points2, CvMat fundamental_matrix, FundamentalMatMethod method)
        {
            return FindFundamentalMat(points1, points2, fundamental_matrix, method, 3.0, 0.99, null);
        }
#if LANG_JP
        /// <summary>
        /// 2枚の画像間の点対応から基礎行列（F行列）を計算する
        ...
        /// <returns></returns>
#endif
        public static int FindFundamentalMat(CvMat points1, CvMat points2, CvMat fundamental_matrix, FundamentalMatMethod method, double param1, double param2)
        {
            return FindFundamentalMat(points1, points2, fundamental_matrix, method, param1, param2, null);
        }
#if LANG_JP
        /// <summary>
        /// 2枚の画像間の点対応から基礎行列（F行列）を計算する
        ...
        /// <returns></returns>
#endif
        public static int FindFundamentalMat(CvMat points1, CvMat points2, CvMat fundamental_matrix, FundamentalMatMethod method, double param1, double param2, CvMat status)
        {
            if (points1 == null)
            ...
            return CvInvoke.cvFindFundamentalMat(points1.CvPtr, points2.CvPtr, fundamental_matrix.CvPtr, method, param1, param2, statusPtr);
        }
        #endregion
        ...
        /// <returns>The function cvFindGraphEdge finds the graph edge connecting two specified vertices and returns pointer to it or NULL if the edge does not exists.</returns>
#endif
        public static CvGraphEdge GraphFindEdge(CvGraph graph, int start_idx, int end_idx)
        {
            return FindGraphEdge(graph, start_idx, end_idx);
        }
        #endregion
        ...
        /// <returns>The function cvFindGraphEdge finds the graph edge connecting two specified vertices and returns pointer to it or NULL if the edge does not exists.</returns>
#endif
        public static CvGraphEdge GraphFindEdgeByPtr(CvGraph graph, CvGraphVtx start_vtx, CvGraphVtx end_vtx)
        {
            return GraphFindEdgeByPtr(graph, start_vtx, end_vtx);
        }
        #endregion
        ...
        /// <returns></returns>
#endif
        public static int FindHomography(CvMat src_points, CvMat dst_points, CvMat homography)
        {
            return FindHomography(src_points, dst_points, homography, HomographyMethod.Zero, 0, null);
        }
#if LANG_JP
        /// <summary>
        /// 2枚の画像間の射影変換を求める
        ...
        /// <returns></returns>
#endif
        public static int FindHomography(CvMat src_points, CvMat dst_points, CvMat homography, HomographyMethod method)
        {
            return FindHomography(src_points, dst_points, homography, method, 0, null);
        }
#if LANG_JP
        /// <summary>
        /// 2枚の画像間の射影変換を求める
        ...
        /// <returns></returns>
#endif
        public static int FindHomography(CvMat src_points, CvMat dst_points, CvMat homography, HomographyMethod method, double ransacReprojThreshold)
        {
            return FindHomography(src_points, dst_points, homography, method, ransacReprojThreshold, null);
        }
#if LANG_JP
        /// <summary>
        /// 2枚の画像間の射影変換を求める
        ...
        /// <returns></returns>
#endif
        public static int FindHomography(CvMat src_points, CvMat dst_points, CvMat homography, HomographyMethod method, double ransacReprojThreshold, CvMat mask)
        {
            if (src_points == null)
                throw new ArgumentNullException("src_points");
            if (dst_points == null)
                throw new ArgumentNullException("dst_points");
            if (homography == null)
                throw new ArgumentNullException("homography");
            IntPtr maskPtr = (mask == null) ? IntPtr.Zero : mask.CvPtr;
            return CvInvoke.cvFindHomography(src_points.CvPtr, dst_points.CvPtr, homography.CvPtr, method, ransacReprojThreshold, maskPtr);
        }
        #endregion
        ...
        /// <returns></returns>
#endif
        public static CvTypeInfo FindType(string type_name)
        {
            if (string.IsNullOrEmpty(type_name))
            {
                throw new ArgumentNullException("type_name");
            }
            IntPtr result = CvInvoke.cvFindType(type_name);
            if (result == IntPtr.Zero)
                return null;
            else
...
#endif
        public static void FitLine(CvArr points, DistanceType dist_type, double param, double reps, double aeps, float[] line)
        {
            if (points == null)
            ...
            CvInvoke.cvFitLine(points.CvPtr, dist_type, param, reps, aeps, line);
        }
        #endregion
        #region Flip
#if LANG_JP
...
#endif
        public static void Flip(CvArr src, CvArr dst, FlipMode flip_mode)
        {
            if (src == null)
            {
                throw new ArgumentNullException("src");
            ...
            CvInvoke.cvFlip(src.CvPtr, dstPtr, flip_mode);
        }
#if LANG_JP
        /// <summary>
        ...
        /// <param name="flip_mode">Specifies how to flip the array.</param>
#endif
        public static void Mirror(CvArr src, CvArr dst, FlipMode flip_mode)
        {
